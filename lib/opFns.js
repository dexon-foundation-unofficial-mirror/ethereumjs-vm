...
module.exports = {
  ...
  SIGNEXTEND: function (k, val, runState) {
    if (k.lten(31)) {
      const signBit = k.muln(8).iaddn(7)
//      const mask = utils.MAX_INTEGER.shln(k.toNumber()).sub(utils.MAX_INTEGER)
      const mask = utils.MAX_INTEGER.shrn(255 - signBit).shln(255 - signBit)

      if (val.testn(k.toNumber())) {
        return val.or(mask)
      } else {
        return new BN(val)
      }
